...
@implementation MWMediatorNavigator

+(MWMediatorNavigator *)navigator{
    static MWMediatorNavigator *rootNavigator = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (rootNavigator == nil) {
            rootNavigator = [[MWMediatorNavigator alloc] init];
        }
    });
...
-(void)pushViewController:(nonnull UIViewController *)controller
    ...
    if (![controller isKindOfClass:[UIViewController class]]) {
        NSLog(@"===== 没有获取到UIViewController 请检查 路由路径是否正确 ViewController = %@",NSStringFromClass([baseViewController class]));
        return;
    }
...
@implementation MWMediatorNavigator (HookRouteBlock)
